...
opa(P,E) :- maennlich(P), kind(X,P), kind(E, X). 
						
% X ist Bruder von Y						
bruder(X,Y) :- 	maennlich(X), kind(X, Z1), kind(X, Z2), kind(Y, Z1), kind(Y, Z2), verheiratet(Z1, Z2), X \= Y.

% X ist Schwester von Y
schwester(X,Y) :- weiblich(X), kind(X, Z1), kind(X, Z2), kind(Y, Z1), kind(Y, Z2), verheiratet(Z1, Z2), X \= Y.
									
% X Ist Cousin von Y									
cousin(X,Y) :- 	maennlich(X), opa(P, X), opa(P, Y), not(schwester(X, Y)), not(bruder(X, Y)), X \= Y.
